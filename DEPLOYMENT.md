...
1. Push the project to **GitHub** (both Vercel and Render deploy from Git).
...
4. In the service **Environment** tab, set:
   ...
   | Variable | Value |
   |----------|--------|
   | `FRONTEND_URL` | Your Vercel URL (e.g. `https://plotcrm.vercel.app`) — set after step 2 |
...
6. **Environment variables**:
   ...
   | `VITE_FRONTEND_URL` | `https://YOUR-APP.vercel.app` (optional, for links) |
...
1. Copy your Vercel URL (e.g. `https://plotcrm.vercel.app`).
...
| `FRONTEND_URL` | Yes (prod) | Vercel URL for CORS & QR links |
...
| CORS error in browser console | Set `FRONTEND_URL` on Render to exact Vercel URL (https, no trailing slash) and redeploy API. |
